...
app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///found.sqlite'
...
GMAIL_APP_PASSWORD = 'your_app_password_here'  # Use App Password
...
def register():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            send_email(email, 'Registration Successful', f'Hello {username}, your account has been created.')
...
def login():
    ...
    if request.method == 'POST':
        ...
        if user and check_password_hash(user.password_hash, password) and not user.is_admin:
            ...
            send_email(user.email, 'Login Alert', f'Hello {user.username}, you have just logged in.')
            return redirect(url_for('index'))
...
@login_required
def logout():
    send_email(current_user.email, 'Logout Alert', f'Hello {current_user.username}, you have logged out.')
    logout_user()
...
def admin_signup():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            send_email(email, 'Admin Account Created', f'Hello {username}, your admin account is ready.')
...
def admin_login():
    ...
    if request.method == 'POST':
        ...
        if admin and check_password_hash(admin.password_hash, password) and admin.is_admin:
            ...
            send_email(admin.email, 'Admin Login', f'Hello {admin.username}, you have logged in as Admin.')
            return redirect(url_for('admin_dashboard'))
...
@admin_required
def admin_dashboard():
    send_email(current_user.email, 'Dashboard Access', 'Admin accessed the dashboard.')
...
def report_found():
    if request.method == 'POST':
        ...
        try:
            ...
            send_email(current_user.email, 'Item Reported', f'You reported the item "{item_name}".')
            return redirect(url_for('index'))
        except Exception:
            db.session.rollback()
...
def claim_item(item_id):
    ...
    try:
        ...
        send_email(current_user.email, 'Item Claimed', f'You claimed "{item.item_name}". Finder contact: {finder.email}')
        send_email(finder.email, 'Your Item Claimed', f'Your item "{item.item_name}" was claimed by {current_user.username} ({current_user.email}).')
    except Exception:
        db.session.rollback()
